...
export class SilverConfidenceScorer {
    logger;
    // Base confidence scores by extraction method
    static METHOD_BASE_SCORES = {
        css_selector: 90,
        regex_pattern: 75,
        manual_rule: 60,
    ...
    scoreExtraction(extractedData, extractionMethods) {
    ...
    calculateFieldConfidences(extractedData, extractionMethods) {
        return {
    ...
    scoreSchoolNameField(data, methods) {
        if (!data.school_name)
            return 0;
    ...
    scoreRankingsField(data, methods) {
        ...
        if (hasNational) {
            ...
            if (rank <= 100)
                nationalConfidence += 5;
            if (rank <= 1000)
                nationalConfidence += 3;
    ...
    scoreAcademicsField(data, methods) {
    ...
    scoreDemographicsField(data, methods) {
    ...
    scoreLocationField(data, methods) {
    ...
    scoreEnrollmentField(data, methods) {
     ...
     */
    static getConfidenceLevelDescription(confidence) {
        if (confidence >= 90)
            return 'Very High';
        if (confidence >= 80)
            return 'High';
     ...
     */
    static meetsMinimumThreshold(fieldConfidences, overallConfidence, threshold = 60) {
        // Overall confidence must meet threshold
        if (overallConfidence < threshold)
            return false;
        // Critical fields must have reasonable confidence
        if (fieldConfidences.school_name < 40)
            return false; // School name is critical
