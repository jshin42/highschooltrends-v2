...
import { healthMonitor } from '../common/health';
import { HealthServer } from '../common/health-server';
...
export class BronzeService {
    ...
    errorRecovery;
    processingStats;
    constructor(config, dbPath, healthServerOptions) {
        this.config = config;
    ...
    async initialize() {
        ...
        try {
            ...
            timer.endWithError(error, 'Failed to initialize Bronze Layer Service');
            throw error;
        }
    }
    /**
    ...
    async processDataset() {
        ...
        try {
            ...
            timer.endWithError(error, 'Dataset processing failed');
            throw error;
        }
    }
    /**
     ...
     */
    async getRecordBySlug(schoolSlug) {
     ...
     */
    async updateRecordStatus(id, status, errors) {
        try {
            ...
            this.logger.error('Error updating record status', error, {
                recordId: id,
                attemptedStatus: status
            });
            throw error;
    ...
    async recoverAllErrors() {
        ...
        try {
            ...
            timer.endWithError(error, 'Comprehensive error recovery failed');
            throw error;
        }
    }
    /**
    ...
    async recoverDriveHealth() {
        ...
        try {
            ...
            timer.endWithError(error, 'Drive health recovery failed');
            throw error;
        }
    }
    /**
    ...
    async reprocessFailedFiles() {
        ...
        try {
            ...
            timer.endWithError(error, 'Failed file reprocessing failed');
            throw error;
        }
    }
    /**
    ...
    async registerHealthChecks() {
        ...
        healthMonitor.registerComponent('external_drives', async () => {
            try {
        ...
        healthMonitor.registerMetric('processing_rate_files_per_min', async () => {
        ...
        healthMonitor.registerMetric('error_rate_percentage', async () => {
        ...
        healthMonitor.registerMetric('p95_processing_time_ms', async () => {
        ...
        healthMonitor.registerMetric('memory_usage_mb', async () => {
            const usage = process.memoryUsage();
            return Math.round(usage.heapUsed / 1024 / 1024);
        });
        healthMonitor.registerMetric('memory_usage_percentage', async () => {
            const usage = process.memoryUsage();
            return Math.round((usage.heapUsed / usage.heapTotal) * 100);
        });
        // Register error recovery metrics
        healthMonitor.registerMetric('error_recovery_success_rate', async () => {
        ...
        healthMonitor.registerMetric('average_recovery_time_ms', async () => {
            const metrics = this.errorRecovery.getRecoveryMetrics();
            return Math.round(metrics.averageRecoveryTime);
        });
        // Register circuit breaker metrics
        healthMonitor.registerMetric('circuit_breaker_open_count', async () => {
        ...
        healthMonitor.registerMetric('circuit_breaker_failure_rate', async () => {
            const metrics = this.processor.getCircuitBreakerMetrics();
        ...
        healthMonitor.registerComponent('circuit_breakers', async () => {
            try {
                const metrics = this.processor.getCircuitBreakerMetrics();
                const openCircuits = [];
                let totalOperations = 0;
    ...
    async shutdown() {
        ...
        try {
            ...
            timer.endWithError(error, 'Error during Bronze Layer Service shutdown');
            throw error;
        }
    }
}
 ...
 */
export function createBronzeService(overrides, dbPath, healthServerOptions) {
    const defaultConfig = {
 ...
 */
export function createBronzeServiceWithHealth(overrides, dbPath, healthPort = 3001) {
    const healthServerOptions = {
        port: healthPort,
...
//# sourceMappingURL=bronze-service.js.map