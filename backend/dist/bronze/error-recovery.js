...
export class BronzeErrorRecovery {
    ...
    async recoverAllFailedRecords() {
        ...
        try {
            ...
            for (const [errorType, records] of Object.entries(recordsByErrorType)) {
                ...
                const result = await this.recoverRecordsByErrorType(errorType, records);
     ...
     */
    async recoverRecordsByErrorType(errorType, records) {
     ...
     */
    async recoverSingleRecord(record, policy) {
     ...
     */
    async handleSpecialErrorType(errorType, records) {
        const startTime = Date.now();
        switch (errorType) {
            case 'duplicate_slug':
     ...
     */
    async handleDuplicateSlugRecords(records) {
        const startTime = Date.now();
        let successful = 0;
        let stillFailed = 0;
        const errors = [];
        ...
        for (const [slug, duplicateRecords] of slugGroups) {
            try {
                ...
                const sortedByTimestamp = duplicateRecords.sort((a, b) => new Date(b.capture_timestamp).getTime() - new Date(a.capture_timestamp).getTime());
     ...
     */
    async attemptDriveRecovery(directory) {
     ...
     */
    groupRecordsByErrorType(records) {
        const groups = {};
        for (const record of records) {
     ...
     */
    extractErrorTypeFromMessage(errorMessage) {
