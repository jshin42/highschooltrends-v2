...
import { BronzeConfiguration } from '../bronze/types';
...
program
  ...
  .option('-d, --directories <dirs>', 'Comma-separated source directories', 
    '/Volumes/OWC Express 1M2/USNEWS_2024,/Volumes/OWC Express 1M2/USNEWS_2025')
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const config: Partial<BronzeConfiguration> = {};
      ...
      Object.entries(stats.files_by_status).forEach(([status, count]) => {
        const statusIcon = {
          ...
          quarantined: '🔒',
          skipped: '⏭️'
        }[status as keyof typeof stats.files_by_status] || '📄';
      ...
      Object.entries(stats.files_by_priority).forEach(([priority, count]) => {
        if (count > 0) {
          const priorityIcon = {
            ...
            bucket_3: '🥉',
            unknown: '❓'
          }[priority as keyof typeof stats.files_by_priority] || '📄';
