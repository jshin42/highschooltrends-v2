...
async function testHealthMonitoring() {
  ...
  try {
      ...
      try {
        ...
        if (endpoint.includes('/health/detailed') && data) {
          ...
          logger.info('Detailed health check response', {
            ...
            components: data.components?.map((c: any) => ({ name: c.name, status: c.status })),
        ...
        logger.error('Health endpoint test failed', error as Error, { endpoint });
      }
    }
    ...
    try {
      ...
      logger.error('Post-processing health check failed', error as Error);
    }
    ...
    logger.error('Health monitoring test failed', error as Error);
    process.exit(1);
  }
}
