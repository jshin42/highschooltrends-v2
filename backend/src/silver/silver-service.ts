...
export class SilverService {
  ...
  constructor(databasePath: string = './data/highschooltrends.db') {
  ...
  async createRecord(record: Omit<SilverRecord, 'id' | 'created_at' | 'updated_at'>): Promise<number> {
    ...
    try {
      ...
      const result = stmt.run(
        ...
        record.state_rank,
        record.ap_participation_rate,
        record.ap_pass_rate,
        record.math_proficiency,
        record.reading_proficiency,
        record.science_proficiency,
        record.graduation_rate,
        record.college_readiness_index,
        record.white_pct,
        record.asian_pct,
        record.hispanic_pct,
        record.black_pct,
        record.american_indian_pct,
        record.two_or_more_pct,
        record.female_pct,
        record.male_pct,
        record.economically_disadvantaged_pct,
        record.free_lunch_pct,
        record.reduced_lunch_pct,
        record.extraction_status,
        record.extraction_confidence,
        JSON.stringify(record.field_confidence),
        JSON.stringify(record.processing_errors)
      );
  ...
  async getRecordById(id: number): Promise<SilverRecord | null> {
  ...
  async getRecordByBronzeId(bronzeRecordId: number): Promise<SilverRecord | null> {
    const stmt = this.db.prepare('SELECT * FROM silver_records WHERE bronze_record_id = ?');
    const result = stmt.get(bronzeRecordId) as any;
  ...
  async getRecordsByStatus(status: ExtractionStatus): Promise<SilverRecord[]> {
  ...
  async getRecordsBySchoolSlug(schoolSlug: string): Promise<SilverRecord[]> {
    const stmt = this.db.prepare('SELECT * FROM silver_records WHERE school_slug = ?');
    const results = stmt.all(schoolSlug) as any[];
  ...
  async updateRecord(id: number, updates: Partial<SilverRecord>): Promise<void> {
  ...
  async createRecordsBatch(records: Omit<SilverRecord, 'id' | 'created_at' | 'updated_at'>[]): Promise<number[]> {
    ...
    try {
      ...
      const transaction = this.db.transaction((recordsToProcess: typeof records) => {
        const insertedIds: number[] = [];
        
        for (const record of recordsToProcess) {
          const result = stmt.run(
            record.bronze_record_id, record.school_slug, record.source_year,
            record.school_name, record.nces_id, record.grades_served,
            record.address_street, record.address_city, record.address_state, record.address_zip,
            record.phone, record.website, record.setting,
            record.enrollment, record.student_teacher_ratio, record.full_time_teachers,
            record.national_rank, record.state_rank,
            record.ap_participation_rate, record.ap_pass_rate, record.math_proficiency,
            record.reading_proficiency, record.science_proficiency, record.graduation_rate,
            record.college_readiness_index,
            record.white_pct, record.asian_pct, record.hispanic_pct, record.black_pct,
            record.american_indian_pct, record.two_or_more_pct, record.female_pct, record.male_pct,
            record.economically_disadvantaged_pct, record.free_lunch_pct, record.reduced_lunch_pct,
            record.extraction_status, record.extraction_confidence,
            JSON.stringify(record.field_confidence),
            JSON.stringify(record.processing_errors)
          );
  ...
  async getStatistics(): Promise<SilverStatistics> {
    ...
    const yearStats = yearStmt.all() as any[];
    const records_by_year: Record<number, number> = {};
  ...
  async getHealthCheck(): Promise<SilverHealthCheck> {
  ...
  private deserializeRecord(row: any): SilverRecord {
    return {
      ...row,
      field_confidence: JSON.parse(row.field_confidence || '{}') as FieldConfidence,
      processing_errors: JSON.parse(row.processing_errors || '[]') as ExtractionError[]
    };
  }
